...
| check                                               | floor                   | where                        |
| --------------------------------------------------- | ----------------------- | ---------------------------- |
| ink on ground, and ink on its own fill              | WCAG 1.4.3 AA, 4.5:1    | `src/checks/contrast.ts`     |
| a meaningful graphic on ground (`scale-empty`)      | WCAG 1.4.11, 3:1        | `src/checks/contrast.ts`     |
...
It is not an escape hatch. The check fails if the pair now passes (stale), if it has
slipped below the recorded ratio (rotting), if the pair is BETTER than the recorded
...
| `shadow-[0_6px_18px_rgba(0,0,0,0.4)]` (the tilted band, written inline) | `--shadow-band`                                     |                                                                                                                                                                                                                            |
...
outside a preset here. It is not a fourth step of the `sm`/`md`/`lg` ramp - it is
wider and softer than `md` at a lower alpha, because it is read against whatever
the band happens to cross - so it became its own depth role. Arcade carries the
upstream value exactly (`0 6px 18px rgba(0, 0, 0, 0.4)`); `light` sits between its
own `md` and `lg`.
